...
class Jp2kr(Jp2kBox):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not all(value is None for _, value in kwargs.items()):
            # if any keyword arguments were not None, then we are writing
            self._mode = 'w'
        elif self.path.exists():
            # either just reading or appending
            self._mode = 'r+'
        else:
        ...
        elif self._mode.startswith('r'):
            # if the file does not exist, then creating it from scratch
            # is the only viable option
            #
            # This seems problematic.  if a file does not exist, a reader
...
class Jp2k(Jp2kr):
    ...
    def __init__(
        ...
        super().__init__(filename, data=data, shape=shape, tilesize=tilesize)
        ...
        if self._mode == 'w' and shape is not None:
            self.shape = shape
        ...
        if self._mode == 'w' and data is None:
    ...
    def _validate_kwargs(self):
        ...
        if self._mode.startswith('r') and self._capture_resolution is not None:
            msg = (
