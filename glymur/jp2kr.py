...
from . import core, get_option, version
...
class Jp2kReader(Jp2kBox):
    """Access JPEG 2000 files.

    Parameters
    ----------
    filename : str or Path
        Path to JPEG 2000 file.
    ...
    def __repr__(self):
        msg = f'Jp2kReader("{self.path.name}", verbose={self.verbose})'
        return msg
    ...
    def _initialize_shape(self):
        ...
        if num_components == 1:
            ...
            self._shape = (height, width, num_components)
    ...
    def _validate_nonzero_image_size(self, nrows, ncols, component_index):
        ...
        The openjpeg library will catch this in version 2.4.
        """
        if nrows == 0 or ncols == 0:
            # Letting this situation continue would segfault openjpeg.
            msg = f"Component {component_index} has dimensions 0 x 0."
            raise InvalidJp2kError(msg)
